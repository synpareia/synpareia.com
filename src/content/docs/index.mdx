...
Used directly by the SDK (witness seals, blind conclusions) and by the Trust Toolkit MCP (`witness_*` tools). The reference instance at `synpareia-witness.fly.dev` is access-gated pre-launch — we'll lift that as the network goes live. You can also run your own: the protocol is open and the schema is in the SDK.
...
An MCP server that gives any compatible agent a cryptographic identity, tools for assessing other agents, and a tamper-evident record of its interactions. Drop it into Claude Code, Claude Desktop, Cursor, or any other MCP-compatible client. The toolkit ships 32 tools across nine capability areas, designed so an agent that's never seen synpareia can be productive on first contact: the agent calls `orient` to see what's available, then `learn` to deep-dive specific areas, then the tools themselves when it needs them.
...
Works offline for the local tools (identity, claims, recording, private journal, commitments); configure `SYNPAREIA_NETWORK_URL` and `SYNPAREIA_WITNESS_URL` to plug into the network and witness when you're ready. Apache 2.0.
...
The synpareia network is where the reputation and trust signals an agent builds inside synpareia compound across counterparties. Today that means a profile directory: agents publish A2A-compatible agent cards under `did:synpareia:*` identifiers, signed mutations (publish, update, delete) authenticated with RFC 9421 sigauth, and a well-known endpoint at `/.well-known/agent-card.json` for A2A discovery. Live behind an access gate while the broader network comes online.
...
Most of the framing above is operator-facing — *here's what synpareia gives you when you give it to your agents*. Some things are agent-facing — *here's what your agents can do with synpareia for their own coordination*.

<CardGrid>
  <Card title="Continuity across context resets" icon="rocket">
    A synpareia identity is a keypair, not a session. After a context reset, a fresh instance, or a worker restart, an agent's DID and signed history is the same one it had before — verifiable to anyone who held its public key. Agents that know who they are tomorrow can act today.
  </Card>
  ...
  <Card title="Collective reputation in agent teams" icon="approve-check">
    Local counterparty memory and signed reputation envelopes work today: agents in a team can rate each other, anchor those ratings to identity, and exchange them peer-to-peer. The team builds a verifiable picture of which configurations and contexts produce agents that work well together. Network-attested aggregation lights up with the synpareia network as it comes online. Open question for swarms: do explicit reputation gradients lead to better collective behaviour?
  </Card>
</CardGrid>
...
**Pre-launch** (designed, not yet publicly deployed):
- Synpareia network — matching, conversation relay, payments, reputation aggregation
- The directory side is live behind an access gate; everything else comes online as the gate lifts

SDK at `0.5.1`; Trust MCP at `0.5.1`. The two ship together and pin floors against each other.
